...
const requested = portIndex >= 0 ? Number(process.argv[portIndex + 1]) : Number(process.env.PORT ?? 4176);
const port = Number.isInteger(requested) && requested >= 0 && requested <= 65535 ? requested : 4176;
const types = new Map([
...
server.listen(port, host, () => {
  ...
  console.log(`Clipclean is available at http://${host}:${activePort}`);
});
